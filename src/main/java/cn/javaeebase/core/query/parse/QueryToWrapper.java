package cn.javaeebase.core.query.parse;
...
import cn.javaeebase.core.query.data.Condition;
import cn.javaeebase.core.query.data.Condition.Filter;
import cn.javaeebase.core.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.javaeebase.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.javaeebase.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.javaeebase.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.javaeebase.core.query.data.Sort.Direction.ASC) {
